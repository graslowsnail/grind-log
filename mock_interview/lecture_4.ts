...
const calculateProfit = (price:number, numberSold:number, cost:number):number => {
  let profit = 0
  profit = price * numberSold - cost
  return profit
}

const profitableItems = (day:Item[], target: number):string[] => {
  let profitList = []
  for(const item of day){
    let profit =  calculateProfit(item.price,item.sold, item.cost)
    if(profit >= target){
      profitList.push(item.name)
    }
  }
  return profitList
}
